# Authors and Citation

## Authors

- **Athanasia Mo Mowinckel**. Author, maintainer.
  [](https://orcid.org/0000-0002-5756-0223)

- **RLadies+**. Copyright holder.
  [![ROR](https://raw.githubusercontent.com/ror-community/ror-logos/main/ror-icon-rgb.svg)](https://ror.org/https://ror.org/05wpb1k41)

## Citation

Source:
[`DESCRIPTION`](https://github.com/rladies/spellbind/blob/main/DESCRIPTION)

Mowinckel A (2026). *spellbind: RLadies+ Brand Toolkit for R*. R package
version 0.1.0, <https://rladies.org/spellbind/>.

    @Manual{,
      title = {spellbind: RLadies+ Brand Toolkit for R},
      author = {Athanasia Mo Mowinckel},
      year = {2026},
      note = {R package version 0.1.0},
      url = {https://rladies.org/spellbind/},
    }
